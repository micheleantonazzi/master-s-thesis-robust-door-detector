...
if __name__ == '__main__':
    ...
    for index, exp in enumerate([EXP_1_HOUSE_7, EXP_2_HOUSE_7_25, EXP_2_HOUSE_7_50, EXP_2_HOUSE_7_75]):
        ...
        metrics = evaluator.get_metrics(iou_threshold=0.75, confidence_threshold=0.5, plot_curves=True, colors=COLORS)
        mAP = 0
        ...
        for i, (label, values) in enumerate(sorted(metrics['per_bbox'].items(), key=lambda v: v[0])):
            ...
            metrics_table['AP'][index * 3 + i] = values['AP']
            metrics_table['Positives'][index * 3 + i] = values['total_positives']
            metrics_table['TP'][index * 3 + i] = values['TP']
            metrics_table['FP'][index * 3 + i] = values['FP']
    ...
    with pd.ExcelWriter('./../results/'+house + '.xlsx') as writer:
