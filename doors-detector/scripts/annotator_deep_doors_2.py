...
dataset_load_path = '/home/michele/myfiles/deep_doors_2'
dataset_save_path = '/home/michele/myfiles/deep_doors_2_labelled'
folder_name = 'deep_doors_2'
folder_manager_save = DatasetFolderManager(dataset_path=dataset_save_path, folder_name=folder_name, sample_class=DoorSample)
dataset_creator = DatasetCreatorDeepDoors2(dataset_path=dataset_load_path)

load_dataset, _ = dataset_creator.creates_dataset(train_size=2999, test_size=1)
possible_labels = sorted(list(DOOR_LABELS.keys()))
colors = {0: (0, 0, 255), 1:(255, 0, 0), 2: (0, 255, 0)}
class_index = 0
img_absoulte_counts = [i for i in range(len(load_dataset))]
img_indexes_iter = iter(i for i in range(len(load_dataset)))
...
def next_img_index():
    ...
    _, _, sample = load_dataset[img_index]
    sample.set_depth_data(sample.get_depth_image().copy())
    sample.set_robot_pose({})
    sample.set_semantic_image(sample.get_pretty_semantic_image())

    img_objects = sample.get_bounding_boxes().tolist()
...
while True:
    ...
    if pressed_key == ord('a') or pressed_key == ord('d'):
        ...
        # show next image key listener
            ...
            folder_manager_save.save_sample(sample, use_thread=False)
            next_img_index()
