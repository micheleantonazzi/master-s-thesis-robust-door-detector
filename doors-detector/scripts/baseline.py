...
images = [cv2.imread('door_med_res.jpg')]
for i, (image, target, sample) in enumerate(test):
    ...
    image = cv2.GaussianBlur(image, (5, 5), sigmaX=0.8, sigmaY=0.8)
    #cv2.imshow('', image)
    #cv2.waitKey()

    canny = cv2.Canny(image, 10, 80, L2gradient=False)
    ...
    corners_locations = harris > 0.001 * harris.max()
    c = 2
    ...
    # Thresholds
    height_thresh_L = 0.5
    height_thresh_H = 0.99
    width_thresh_L = 0.1
    width_thresh_H = 0.9
    direction_thresh_L = 10
